...
def find_many_by_field(collection: Any, field: str, value: Any):
  results: List[Dict[str, Any]] = []
  seen_ids = set()
  for candidate in id_candidates(value):
    for item in coerce_list(collection.find({field: candidate})):
...
def find_one_by_field(collection: Any, field: str, value: Any, projection: Optional[Dict[str, int]] = None):
  for candidate in id_candidates(value):
    document = collection.find_one({field: candidate}, projection)
    if document:
      return document
  return None


def get_reviews_for_product(product_id: Any):
  return find_many_by_field(product_reviews, "product_id", product_id)
...
def serialize_product(product: Dict[str, Any]):
  ...
  product_reviews_list = get_reviews_for_product(product.get("_id"))
...
def generate_artisan_recommendations(user_id: str):
  artisan = find_one_by_field(artisans, "userId", user_id)
...
def attach_artisan_user(product: Dict[str, Any]):
  ...
  artisan_user = find_one_by_field(users, "_id", artisan_id, {"email": 1})
  hydrated = dict(product)
  hydrated["artisan_id"] = artisan_user or to_object_id(artisan_id) or artisan_id
  return hydrated


def attach_artisan_profile_user(artisan: Dict[str, Any]):
  user_id = artisan.get("userId")
  artisan_user = find_one_by_field(users, "_id", user_id, {"email": 1, "role": 1})
  hydrated = dict(artisan)
  hydrated["userId"] = artisan_user or to_object_id(user_id) or user_id
  return hydrated


def attach_review_user(review: Dict[str, Any]):
  user_id = review.get("user_id")
  reviewer = find_one_by_field(users, "_id", user_id, {"email": 1, "fullName": 1, "role": 1})
  hydrated = dict(review)
  hydrated["user_id"] = reviewer or to_object_id(user_id) or user_id
  return hydrated
...
async def get_products(artisan_id: Optional[str] = None):
  if artisan_id:
    items = find_many_by_field(products, "artisan_id", artisan_id)
  else:
    items = coerce_list(products.find({}))
  return [serialize_product(attach_artisan_user(product)) for product in items]
...
async def get_product_reviews(product_id: str):
  ...
  items = [attach_review_user(review) for review in get_reviews_for_product(object_id)]
...
async def artisan_profile(authorization: Optional[str] = Header(default=None)):
  user = require_user(authorization)
  artisan = find_one_by_field(artisans, "userId", user["id"])
  if not artisan:
...
async def get_artisans(userId: Optional[str] = None):
  if userId:
    items = find_many_by_field(artisans, "userId", userId)
  else:
    items = coerce_list(artisans.find({}))
  return [serialize_artisan(attach_artisan_profile_user(artisan)) for artisan in items]
...
async def get_orders(authorization: Optional[str] = Header(default=None)):
  user = require_user(authorization)
  items = []
  for order in find_many_by_field(orders, "buyer_id", user["id"]):
    hydrated = dict(order)
    hydrated["product_id"] = find_one_by_field(products, "_id", order.get("product_id"))
